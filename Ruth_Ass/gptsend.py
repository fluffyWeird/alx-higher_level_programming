...
def listen_for_messages(sock):
    ...
    while True:
        try:
            message, _ = sock.recvfrom(2048)  # Buffer size can be adjusted if needed
            print("\nReceived:", message.decode())  # Displays messages received from the server

            # Check if this is a port assignment message
            if message.decode().startswith("OK Hello"):
                # Extract the new port number if provided in the format "OK Hello Username at Port"
                new_port = int(message.decode().split()[-1])
                if new_port:
